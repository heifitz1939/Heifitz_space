...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (uLineStyle > 0.5) {
        float dr = 4.0; float dd = 0.5;
        float vy = fract(gridUV.y*dr); float vx = fract(gridUV.x*dr);
        float dmY = step(vy, dd); float dmX = step(vx, dd);
        if (uLineStyle < 1.5) { lineX *= dmY; lineY *= dmX; }
        else {
            float dtr = 6.0; float dw = 0.18;
            float cy = abs(fract(gridUV.y*dtr)-0.5); float cx = abs(fract(gridUV.x*dtr)-0.5);
            lineX *= 1.0 - smoothstep(dw, dw+fwidth(gridUV.y*dtr), cy);
            lineY *= 1.0 - smoothstep(dw, dw+fwidth(gridUV.x*dtr), cx);
        }
    }
    ...
    if (uLineStyle > 0.5) {
        ...
        float vy2 = fract(gridUV2.y*dr2); float vx2 = fract(gridUV2.x*dr2);
        if (uLineStyle < 1.5) { lineX2 *= step(vy2,dd2); lineY2 *= step(vx2,dd2); }
        else {
            float dtr2 = 6.0; float dw2 = 0.18;
            float cy2 = abs(fract(gridUV2.y*dtr2)-0.5); float cx2 = abs(fract(gridUV2.x*dtr2)-0.5);
            lineX2 *= 1.0 - smoothstep(dw2, dw2+fwidth(gridUV2.y*dtr2), cy2);
            lineY2 *= 1.0 - smoothstep(dw2, dw2+fwidth(gridUV2.x*dtr2), cx2);
        }
    }
    ...
    for (int i = 0; i < MAX_SCANS; i++) {
        ...
        float pI = clamp(tA/dur, 0.0, 1.0);
        if (uScanDirection > 0.5 && uScanDirection < 1.5) { pI = 1.0 - pI; }
        else if (uScanDirection > 1.5) { pI = (pI < 0.5) ? pI*2.0 : 1.0-(pI-0.5)*2.0; }
        float dzI = abs(hit.z - pI*scanZMax);
        float hfI = smoother01(0.0, taper, pI);
        float tfI = 1.0 - smoother01(1.0-taper, 1.0, pI);
        combinedPulse += exp(-0.5*(dzI*dzI)/(sigma*sigma)) * hfI*tfI * clamp(uScanOpacity,0.0,1.0);
...
export const GridScan = ({
  ...
  const bufX = useRef([]); const bufY = useRef([]);
  const bufT = useRef([]); const bufYaw = useRef([]);
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (enableGyro && window.DeviceOrientationEvent?.requestPermission) {
  ...
  useEffect(() => {
    ...
    return () => {
      stop = true;
      if (video) { const s = video.srcObject; if (s) s.getTracks().forEach(t => t.stop()); video.pause(); video.srcObject = null; }
    };
...
function srgbColor(hex) { return new THREE.Color(hex).convertSRGBToLinear(); }

function smoothDampVec2(current, target, currentVelocity, smoothTime, maxSpeed, deltaTime) {
  smoothTime = Math.max(0.0001, smoothTime);
  const omega = 2 / smoothTime, x = omega * deltaTime;
...
function median(buf) {
  ...
  const a = [...buf].sort((x, y) => x - y), mid = Math.floor(a.length / 2);
  return a.length % 2 ? a[mid] : (a[mid - 1] + a[mid]) * 0.5;
}
function centroid(pts) { let x = 0, y = 0; for (const p of pts) { x += p.x; y += p.y; } return { x: x / (pts.length || 1), y: y / (pts.length || 1) }; }
